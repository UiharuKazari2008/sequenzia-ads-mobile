...
//    };
    ...
    public void onDestroy() {
        ...
        if (AmbientDataManager.lastNotification[0] != null) {
            File oldFile = new File(getFilesDir(), String.format("adi-%s", AmbientDataManager.lastNotification[0]));
            try {
        ...
        if (AmbientDataManager.lastNotification[1] != null) {
            File oldFile = new File(getFilesDir(), String.format("adi-%s", AmbientDataManager.lastNotification[1]));
            try {
                boolean delete = oldFile.delete();
                if (oldFile.exists()) {
                    deleteFile(oldFile.getAbsolutePath());
